...
export const DialogDetails = ({ id, isOpen, onClose = () => {} }) => {
  ...
  const price = useMemo(() => {
    ...
    subtotal = _get(receipt, "receipt_items").reduce(
      (p, { price, quantity }) => {
  ...
  return (
    <Dialog
      ...
      title={
        receipt
          ? `Detail Receipt #${_get(receipt, "receipt_number")}`
          : "Loading"
      }
    >
          ...
          <>
            <Box sx={{ mb: 3 }}>
              <Flex>
                ...
                <Box>{moment(_get(receipt, "created_at")).format("LLLL")}</Box>
              </Flex>
              {_get(receipt, "customer") && (
                <Flex sx={{ mt: 2 }}>
                  ...
                  <Box sx={{ textAlign: "right" }}>
                    ...
                    <Box sx={{ color: "gray.5" }}>
                      {_get(receipt, "customer.phone_number")}
                    </Box>
                  </Box>
                </Flex>
              )}
            ...
            <Box>
              <Box className={Classes.CARD} sx={{ px: 0, py: 2 }}>
                <Box>
                  {_get(receipt, "receipt_items").map((item, id) => (
                    <Flex key={id} sx={{ px: 2, mb: 2 }}>
                      <Box sx={{ flexGrow: 1 }}>
